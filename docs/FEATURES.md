...
#### 엑셀 내보내기 (2026-02-10)
- **다운로드 버튼**: 필터 영역 옆 다운로드 아이콘
- **서버 사이드 생성**: `/api/seminars/export` API에서 xlsx 바이너리 생성 → 브라우저 다운로드
- **라이브러리**: `xlsx-js-style` (셀 스타일링 지원)
- **상단 요약 섹션**: 정기/비정기 세미나 누적 회수 (다운로드 날짜 기준)
- **칼럼**: 구분, 날짜, 주관, 세미나명, 장소, 인원, 주차, 접수자, 센터 담당자, 기타(지원 등)
- **스타일**: 헤더 회색 배경 + 볼드체, 전체 셀 thin 그리드 테두리
- **파일명**: `세미나현황_{연도}.xlsx`
- **데이터 매핑**:
  - 정기 세미나: `구분=정기`, `세미나명=title`, `인원=expectedAttendees`
  - 비정기 요청: `구분=비정기`, `주관=requestingCenter`, `접수자=receiver`, `센터 담당자=centerContact`

#### 주차지원여부 필드 (2026-02-10)
- `parkingSupport?: boolean` 필드 추가 (Seminar, SeminarRequest 모두)
- 정기/비정기 세미나 폼에 체크박스 UI 추가
- 엑셀 내보내기 시 "O"/"X"로 표시
...
### 모바일 가독성 최적화 (2026-02-16)
- **최소 폰트 크기 보장**: globals.css에서 Tailwind 기본 크기를 모바일 가독성에 맞게 오버라이드
  - `text-xs` (12px) → 13px
  - `text-sm` (14px) → 15px
  - `text-[7~9px]` → 11px, `text-[10~11px]` → 12px
- **Fluid Typography**: `clamp()` 기반 반응형 폰트 크기 클래스 제공
  - `text-fluid-xl`: clamp(1.25rem, 4vw, 1.5rem)
  - `text-fluid-lg`: clamp(1.125rem, 3.5vw, 1.25rem)
  - `text-fluid-base`: clamp(1rem, 3vw, 1.125rem)
- **글꼴 크기 5단계 조절**: 설정에서 사용자가 선택 가능
  - 작게(14px), 기본(15px), 크게(17px), 아주 크게(19px), 최대(21px)
...
### 2026-02-16
- 모바일 가독성 향상을 위한 최소 폰트 크기 보장:
  - body 기본 폰트: 15px, line-height 1.6 설정
  - text-xs(12px) → 13px, text-sm(14px) → 15px 오버라이드
  - 임의 픽셀 크기 최소값 보장: text-[7~9px] → 11px, text-[10~11px] → 12px
  - Fluid Typography 클래스 추가 (clamp() 기반 3단계)
  - 운명나침반 프로젝트 가독성 기준에 맞춤

### 2026-02-10
- 세미나현황 엑셀 내보내기 기능 추가:
  - xlsx-js-style 라이브러리로 서버 사이드 엑셀 생성
  - 정기/비정기 구분 칼럼, 접수자/센터 담당자 칼럼
  - 상단 요약: 정기/비정기 세미나 누적 회수
  - 헤더 회색 배경 + 볼드, 전체 그리드 테두리
- 주차지원여부(parkingSupport) 필드 추가:
  - Seminar, SeminarRequest 데이터 모델에 추가
  - 정기/비정기 세미나 등록/수정 폼에 체크박스 추가
  - 모든 관련 API 엔드포인트 업데이트
